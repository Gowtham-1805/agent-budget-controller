...
class InMemoryBudgetRepository:
    ...
    async def list_agent_policies(self, tenant_id: str) -> tuple[AgentPolicy, ...]:
        return tuple(p for (t, a), p in self._agent_policies.items() if t == tenant_id)
    ...
    async def list_budget_policies(
        ...
        return tuple(
            p
            for (t, k), p in self._budget_policies.items()
            if t == tenant_id and (scope_type is None or p.scope_type == scope_type)
        )
    ...
    async def list_sessions(
        ...
        sessions = [
            s
            for (t, sid), s in self._sessions.items()
            if t == tenant_id and (agent_id is None or s.agent_id == agent_id)
        ]
